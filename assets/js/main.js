...
var Mashiro = {
	...
	F_NORMALIZE: function () {
		...
		this.F_TOC_BOT();
		this.F_ANIMATE();
		this.F_COMMENT_BOX_INI();
		this.F_AJAX_COMMIT_COMMENT();
		this.F_COMMENT_PROFILE_INSERT();
	...
	F_PJAX_BACK_RELOAD: function () {
		if ($(window).width() > Mashiro.SETTINGS.moblie_max_width) {
			...
			if (this.V_HORIZONTAL_SCROLL_VARBLES.has_thumb_list) {
				this.V_HORIZONTAL_SCROLL_VARBLES.eof_l = document.querySelector('.post-thumb-item-eof').offsetLeft;
				this.F_HORIZONTAL_SCROLL(pjax_back_flag = true);
			}
			this.F_HEADER_BAR_SHOW_TITLE();
	...
	//Elements that not able to add in PHP, so...
	F_COMMENT_PROFILE_INSERT: function () {
		// Comment profile button
		if ($('.form-submit input').hasClass('submit') && $('div').hasClass('comment-user-avatar')) {
			$('#comment-user-avatar').insertBefore('#submit');
			$('#comment-user-avatar').css('display', 'block');
		}
	},

	F_COMMENT_BOX_INI: function () {
		// Textarea shape
		$('body').on('focusin', '#commentform', function () {
			setTimeout(function () {
				$('#comment-body-container, #comment-body, .form-submit, #comment-user-avatar').addClass('focus');
				$('#comment-body').attr('rows', '5');
			}, 50)
		});
		$('body').on('focusout', '#commentform', function () {
			setTimeout(function () {
				$('#cmt-info-container').removeClass('show');
				$('#comment-body-container, #comment-body, .form-submit, #comment-user-avatar').removeClass('focus');
				$('#comment-body').attr('rows', '1');
			}, 50)
		});
		$('body').on('click', '#comment-user-avatar input', function () {
			$('#cmt-info-container').toggleClass('show');
		});
		$('body').on('focusin', '#cmt-info-container', function () {
			setTimeout(function () {
				$('#cmt-info-container').addClass('show');
			}, 50)
		});
		//cmt-popup
		$('body').on('click', '.cmt-popup', function () {
		...
		$('body').on('click', '#comments-navi a', function (e) {
			...
			$.ajax({
				...
				url: $(this).attr('href'),
				beforeSend: function () {
					//remove existing temp ele
					$('#cmt-temp-ele').remove();
					//clean parent value input box
				...
				success: function (out) {
					var result = $(out).find('#comment-wrap'),
						nextlink = $(out).find('#comments-navi');
					$('#loading-comments').slideUp('fast');
					$('#loading-comments').after(result.fadeIn(500));
					$('#comment-footer').prepend(nextlink);
				}
			});
		});
	...
	F_AJAX_COMMIT_COMMENT: function () {
		$(document).on("submit", "#commentform", function () {
			$.ajax({
				...
				error: function (request) {
					...
					Mashiro.F_NOTIFY(request.responseText, 'top-right', 'error');
				},
				success: function (data) {
					$('#comment-body').val('');

					if ($('#comment_parent').val() == 0) {
						$("#comment-wrap").prepend(data);
						$("#comments").scrollTop(0);
					} else {
						//append data to item (before temp ele)
						$('<ul class="children">' + data + '</ul>').insertBefore("#cmt-temp-ele");
						$("#comments").scrollTop($('#cmt-temp-ele').prev().position().top);
						$('#cmt-temp-ele').remove();

						//remove RE title
						$('#reply-to').css('display', 'none');
						$('#cancel-comment-reply-link').css('display', 'none');
						//clean input box (parent value!!)
						$('#comment_parent').val('0');
					}

					$('#submit').removeClass('submitting');
					Mashiro.F_NOTIFY('Commit secceed!', 'top-right', 'success');
					$('#comment-body').blur();
				}
			});
			return false;
		...
		$('body').on('click', '.comment-reply-link', function () {
			moveForm("comment-" + $(this).attr('data-commentid'), $(this).attr('data-commentid'), "respond", $(this).attr('data-postid'));
			return false;
		});
		// @return addComment.moveForm( "comment-65", "65", "respond", "10" )
		var moveForm = function (parentItem, parentId, respondId, postID) {
			//remove existing temp ele
			$('#cmt-temp-ele').remove();
			//clean parent value input box
			...
			//fill in parent value input box
			//$('#comment_post_ID').val(postID);
			$('#comment_parent').val(parentId);
			//show cencel button
			document.getElementById('cancel-comment-reply-link').style.display = "block";
			//reply to...
			var reName = $('#' + parentItem + ' .author-name:first').text(),
				reURL = $('#' + parentItem + ':first').attr('data-cmt-link');
			$("#reply-to").html('Reply to <a href="' + reURL + '"> @' + reName + '</a>: ');
			document.getElementById('reply-to').style.display = "inline-block";

			$('#comment-body').focus();

			//disable a tag jumpping
			return false;
	...
	F_NOTIFY: function (text, type = 'top-left', status = 'notice', show = 'do-show') {
		//Mashiro.F_NOTIFY('Hello World')
		//type: top-left top-right  bottom-right bottom-left bar-top bar-bottom
		//status: success warning error notice plain
		$('#notify')
			.html('')
			.attr('class', 'notify')
			.attr('data-notification-status', '');
		setTimeout(function () {
			$('#notify')
				.html('<span>' + text + '</span>')
	...
	F_MOBILE_OPEN_NAV: function () {
		$('body').on("click", '#mobile-open-nav-iconflat', function () {
			//debug('detect click');
	...
	F_READ_MODE: function () {
		$("body").on("click", "#open-read-mode", function () {
			...
			// Rename the clone content ids
			var fixIds = function (elem, cntr) {
				$(elem).find("[id]").each(function () {
					this.id = this.id + cntr;
				})
			}
			fixIds('.entry-content.read-mode', '-read-mode');

			// Redirect in-page jump hash #
			var fixHashs = function (elem, cntr) {
				$(elem).find("a").each(function () {
					var reg = new RegExp(/^(#[^\s#])/i);
					if (reg.test($(this).attr('href'))) {
						$(this).attr('href', $(this).attr('href') + cntr);
					}
				});
			}
			fixHashs('.entry-content.read-mode', '-read-mode');
		});
		$("body").on("click", "#close-read-mode", function () {
			$('#read-mode-container').css({
				'height': '0'
			});
	...
	F_SIDE_BUTTOMS: function () {
		...
		$("body").on("click", "#button-hide-comment", function () {
			$('#comment-box').css('right', '-50vw');
		});
		$("body").on("click", "#nav-go-top", function () {
	...
	F_MOBILE_GO_TOP: function () {
		...
		$("body").on("click", "#mobile-go-top", function () {
			$("html").animate({
				scrollTop: 0
			}, '500');
		});
	...
	F_HORIZONTAL_SCROLL: function (pjax_back_flag = false) {
		var w = $(window).width();
		 ...
		 * the left side of screen. I guess in this condition,
		...
		if (this.V_HORIZONTAL_SCROLL_VARBLES.has_thumb_list) {
			...
			var sroll_event = function (dx, dy) {
